...
bool IOTAppStory::iotUpdater(int command) {
    ...
    bool result = false;
        ...
        } else if(command == U_FILESYSTEM) {
            DEBUG_PRINT(SER_FILESYSTEM);
    ...
    if(command == U_FLASH || command == U_FILESYSTEM) {
        // sketch / file system
...
String IOTAppStory::strRetCertScan(String path) {
    ...
    File root = FILESYSTEM.open("/cert");
...
String IOTAppStory::strRetCertScan(String path) {
    ...
    if(!FILESYSTEM.exists("/cert")){  // || !root.isDirectory()
        #if DEBUG_LVL >= 2
    ...
    // open certificate directory
    Dir dir = FILESYSTEM.openDir("/cert/");
