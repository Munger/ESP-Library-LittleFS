    ...
    */
    if(this->_command == U_FILESYSTEM) {
            ...
            if(this->_upd_size > _partition->size) {
                #ifdef DEBUG_SERIAL_ENABLE
                    dbSerialPrintf("File system update to large! (%d) free: %d\n", this->_upd_size, _partition->size);
                #endif

                (*this->_statusMessage) = SER_UPD_FS_TO_LARGE;
                return false;
            }
        #elif defined ESP8266
            size_t FSSize = ((size_t) &_FS_end - (size_t) &_FS_start);
            if(this->_upd_size > (int) FSSize) {
                #ifdef DEBUG_SERIAL_ENABLE
                    dbSerialPrintf("File system update to large! (%d) free: %d\n", this->_upd_size, FSSize);
                #endif
